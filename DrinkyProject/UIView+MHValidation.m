...
-(void)searchForObjectsOfClass:(NSArray*)classes
    ...
    if (selectionType == MHSelectionTypeNext ) {
        if (objectWhichShouldBecomeFirstResponder) {
            long index = [fieldsSort indexOfObject:objectWhichShouldBecomeFirstResponder];
        ...
        if (!objectWhichShouldBecomeFirstResponder) {
            long index = [fieldsSort indexOfObject:[self findFirstResponderOnView:self]];
        ...
        if (index>=2) {
            ...
            if (index == NSNotFound && [selectedObject isFirstResponder ]) {
                ...
                long firstresponderIndex = [fieldsSort indexOfObject:objectWhichShouldBecomeFirstResponder];
...
-(UISegmentedControl *)prevNextSegment {
    UISegmentedControl*  prevNextSegment = [[UISegmentedControl alloc] initWithItems:@[ NSLocalizedString(@"Back", nil), NSLocalizedString(@"Next", nil) ]];
